...
const OrdersPage = () => {
  ...
  const orders = useCustomerOrdersStore((state) => state.cancelled);
  const getCancelled = useCustomerOrdersStore((state) => state.getCancelled);

  useEffect(() => {
    getCancelled(user?.id!);
  }, []);
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          refreshing={false}
          onRefresh={() => getCancelled(user?.id!)}
        />
      }
    >
