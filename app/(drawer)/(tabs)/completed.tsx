...
const OrdersPage = () => {
  ...
  const orders = useCustomerOrdersStore((state) => state.completed);
  const getCompleted = useCustomerOrdersStore((state) => state.getCompleted);
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl
          refreshing={false}
          onRefresh={() => getCompleted(user?.id!)}
        />
      }
    >
